...
function positionUI() {
    ...
    // 计算相对于 wrapper（canvas 的父元素）的位置
    const leftPx = Math.max(10, Math.floor(imgStartX + imgW - uiWidth - 12));
    const topPx = Math.max(10, Math.floor(imgStartY + imgH - uiHeight - 12));

    ui.style.left = `${leftPx}px`;
    ui.style.top = `${topPx}px`;
}